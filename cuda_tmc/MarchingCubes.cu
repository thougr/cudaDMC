...
int warpReduce(T *i_data, const int N) {
	...
	warp_reduce_kernel<T> << <blocks, threads >> >(i_data, d_sum, N);
    cudaCheckError();
    // return sum
	T h_sum{ 0 };
...
void
p_mc::MarchingCubes::mc_halfedge(const float i0,
                                 int dims[3], vtkDataArray* scalarsArray,
                                 int& nr_v, float** vertices, float** normals, int& nr_t, int4** h_hee, int** h_hev,int** h_hef)
{
	Halfedges he_;
...
//	readDataFromFile(i_file, dims, origin, spacing, h_data);
    h_data.resize(scalars.size());
    for (int index = 0; index < scalars.size(); index++) {
        h_data[index] = scalars[index];
    }
...
void
p_mc::MarchingCubes::mc_sharedvertex(const float i0,
                                     int dims[3], vtkDataArray* scalarsArray,
                                     int& nr_v, float** vertices, float** normals, int& nr_t, int** triangles)
{
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	std::cout << " ... read data from file\n";
